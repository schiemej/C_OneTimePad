...
int main(int argc, char* argv[]) {
    ...
    while (1) {
        ...
        //Holds the recv code unique to the enc client server relationship      
        int recv_code = 0;
        int enc_code = 456;
        int send_check = 1;

        //Challenge and Password
        charsRead = recv(connectionSocket, &recv_code, sizeof(enc_code), 0);
        if (recv_code != enc_code) {
            //If recv code not equal to dec code then print error and skip servers work while continuing to listen.
            perror("ENC SERVER CAN ONLY BE ACCESSED BY ENC CLIENT\n");
        ...
        int read_count = 0;
        //left_to_read keeps track of how many characters remain
        ...
        // Encrpyt file
        int x;
        //Iterates through plaintext file and performs decryption operations
        for (x = 0; x < strlen(buffer_1) - 1; x++) {
            //printf("ENCRYPTION LOOP: INDEX: %d BUFFER:%d\n", x, strlen(buffer_1));
            if (buffer_1[x] == '\0') {
                goto EXIT_LOOP;
            }
            //If buffer is equal to space
            if (buffer_1[x] == 32) {
                goto SKIP;
            }
            int a = buffer_1[x] - 65;
    ...
    EXIT_LOOP:;
        ...
        //Sleep for 1 second, leave this line in or else socket closes too early for client to recv all data
        //Unexplained issued, worked on it with multiple TA's and this was the best solution found
        sleep(1);
        // Close the connection socket for this client
        close(connectionSocket);

        //Reset all buffers uses
        memset(buffer, '\0', sizeof(buffer));
        memset(buffer_1, '\0', sizeof(buffer_1));
        memset(buffer_2, '\0', sizeof(buffer_2));
    CONT_LISTEN:;
